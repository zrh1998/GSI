...
class TransformerPredictor(nn.Module):
    ...
    def feature_generate(self, roi_features, proposals, num_objs, rel_pair_idxs, union_features, num_rels, logger):
        ...
        return pair_pred, obj_dists,pair_pos_embed,visual_rep,obj_feats,pair_obj_embed,num_rels, num_objs,logger,prod_rep
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        #         union_locations.append([sub_inf,obj_inf,union_inf])
        ...
        #     sub_proposal=[]
        #     obj_proposal=[]
        # l=len(union_locations)#len(union_location)=pair_pos_embed行数
        # #建立一个新tensor 全0
        # pos_ten=torch.zeros([l,256])
        # for m in range(l):
        #     # print(m.size()) #256
        #     # print("###########################")
        #     for n in range(l):
        #         #若重合区域不为空
        #         if sum(union_locations[m][2])>0. and sum(union_locations[n][2])>0.:
        #             union_union=calc_coin_location(union_locations[m][2][0],union_locations[m][2][1],union_locations[m][2][2],union_locations[m][2][3],union_locations[n][2][0],union_locations[n][2][1],union_locations[n][2][2],union_locations[n][2][3])
        #             a=(union_union[2]-union_union[0])*(union_union[3]-union_union[1])
        #             b=(union_locations[m][2][2]-union_locations[m][2][0])*(union_locations[m][2][3]-union_locations[m][2][1])
        #             IOU_score=a/b
        #             #如果重合度高，再写入新的tensor中
        #             if IOU_score<1.0 and IOU_score>0.8:
        #                 pos_ten[m]=pair_pos_embed[n]
        # print(pair_pos_embed.size())
        # print(pos_ten.size())
        # print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^")
            ...
            for n in range(l):
                ...
                if sum(union_locations[m][2])>0. and sum(union_locations[n][2])>0.:
                    ...
                    if IOU_score>max:
                        ...
                        flag_max=n
                if flag_max != 999:
                    pos_ten[m]=pair_pos_embed[flag_max]
        # pair_pos_embed=torch.einsum('ik,jk->ij',pair_pos_embed.cuda(),pos_ten.cuda())
