    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        for i in rel_pair_idxs:
            ...
            for j in i:
                ...
                sub_word_embedding = sub_word_embedding.unsqueeze(-1)
                obj_word_embedding = obj_word_embedding.unsqueeze(0)
                # 矩阵乘
                pair_word_embed = torch.matmul(sub_word_embedding, obj_word_embedding).unsqueeze(0).unsqueeze(0)
                # print(pair_word_embed.size())
                pair_word_embed = self.conv_block(pair_word_embed.cuda()).view(2048).cpu().unsqueeze(0)
        ...
        # 单纯语义维度
        pair_word_embeds = torch.cat(pair_word_embeds, 0).to(device)
        # union_features = self.word_feature(torch.cat([pair_word_embeds, union_features], dim=1))

        # #几何维度
        union_proposal = box_union(torch.from_numpy(np.array(subject_pos)), torch.from_numpy(np.array(object_pos))).cuda()
        flag_n=0
        ...
        for k in flag_relations:
            ...
            for m in range(flag_n,flag_n+k):
                ...
                pos_ten[m]=torch.where(max(IOU_score)>0.8,union_features[max_iou.indices+flag_n],pos_ten[m])
                IOU_score.zero_()
            flag_n=k+flag_n
        # 失败的改进
        ...
        # 单纯几何维度
        #union_features=self.pos_feature(torch.cat([pos_ten, union_features], dim=1))

        # 语义维度加几何维度
        union_features=self.position_word_feature(torch.cat([pair_word_embeds, union_features, pos_ten.cuda()], dim=1))
