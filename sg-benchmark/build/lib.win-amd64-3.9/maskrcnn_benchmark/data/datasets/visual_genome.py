...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        box = torch.from_numpy(box).reshape(-1, 4).long()  # guard against no boxes
        if flip_img:
            new_xmin = w - box[:,2]
            new_xmax = w - box[:,0]
            box[:,0] = new_xmin
        ...
        target.add_field("labels", torch.from_numpy(self.gt_classes[index]).long())
        target.add_field("attributes", torch.from_numpy(self.gt_attributes[index]).long())
