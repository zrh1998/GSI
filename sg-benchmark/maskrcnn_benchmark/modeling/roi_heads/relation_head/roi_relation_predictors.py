...
from .utils_relation import box_iou,box_union
...
class TransformerPredictor(nn.Module):
    def __init__(self, config, in_channels):
        ...
        self.conv_block= nn.Sequential(
            nn.Conv2d(1, 64, kernel_size=5,stride=2,padding=2,bias=True),   #100
            nn.ReLU(inplace=True),
            nn.AvgPool2d(2, 2), #50
            nn.Conv2d(64, 128, kernel_size=3,stride=2,padding=1,bias=True), #25
            nn.ReLU(inplace=True),
            nn.AvgPool2d(2, 2),  # 12
            nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1, bias=True),  # 6
            nn.AdaptiveAvgPool2d(1),    #1
            nn.ReLU(inplace=True),
            nn.Conv2d(256, 2048, 1, 1, 0, bias=True)
        )
        #纯语义维度
        self.word_feature = nn.Sequential(*[
            nn.Linear(4096+2048, 512), nn.ReLU(inplace=True), nn.Dropout(0.1),
            nn.Linear(512, 4096), nn.ReLU(inplace=True), nn.Dropout(0.1),
        ])

        # #纯几何维度
        # self.pos_feature = nn.Sequential(*[
        #     nn.Linear(4096+4096, 512), nn.ReLU(inplace=True), nn.Dropout(0.1),
        #     nn.Linear(512, 4096), nn.ReLU(inplace=True), nn.Dropout(0.1),
        # ])

        # #语义维度加几何维度
        # self.position_word_feature = nn.Sequential(*[
        #     nn.Linear(4096+4096+2048, 512), nn.ReLU(inplace=True), nn.Dropout(0.1),
        #     nn.Linear(512, 4096), nn.ReLU(inplace=True), nn.Dropout(0.1),
        # ])
    ...
    # def new_feature_generate(self,visual_rep, obj_feats, pair_obj_embed, pair_pos_embed, num_rels, num_objs,
    #                                          logger,prod_rep):
    #     edge_feats = self.edge_context_layer(visual_rep, obj_feats, pair_obj_embed, pair_pos_embed, num_rels, num_objs,
    #                                          logger)
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        for i in rel_pair_idxs:
            ...
            for j in i:
                ...
                sub_word_embedding = sub_word_embedding.unsqueeze(0).unsqueeze(-1)
                obj_word_embedding = obj_word_embedding.unsqueeze(0).unsqueeze(0)
                ...
                # #记录每batch张图片中第几个关系，也就是第几行

                flag_relation = flag_relation + 1
            flag_relations.append(flag_relation)
            flag_relation = 0
            flag_picture = flag_picture + 1
        
        sub_word_embeddings = torch.cat(sub_word_embeddings, dim=0).cuda()
        obj_word_embeddings = torch.cat(obj_word_embeddings, dim=0).cuda()
        sub_word_embeddings = torch.split(sub_word_embeddings, 256, dim=0)
        obj_word_embeddings = torch.split(obj_word_embeddings, 256, dim=0)
        for sub_word_embedding, obj_word_embedding in zip(sub_word_embeddings, obj_word_embeddings):
            # 矩阵乘
            pair_word_embed = torch.matmul(sub_word_embedding, obj_word_embedding).unsqueeze(1)
            # print(pair_word_embed.size())
            pair_word_embed = self.conv_block(pair_word_embed).view(-1, 2048)
            # pair_word_embed=self.relu(self.conv1(pair_word_embed))
            # 将每个pair按顺序加入到对应图像的word embeddings中
            pair_word_embeds.append(pair_word_embed)

        # 单纯语义维度
        pair_word_embeds = torch.cat(pair_word_embeds, 0)
        union_features = self.word_feature(torch.cat([pair_word_embeds, union_features], dim=1))

        # # # #几何维度
        # union_proposal = box_union(torch.from_numpy(np.array(subject_pos)).cuda(), torch.from_numpy(np.array(object_pos)).cuda())
        # flag_n=0
        # pos_ten=torch.zeros([sum(flag_relations),4096]).cuda()
        # for k in flag_relations:
        #     #针对一张图片
        #     for m in range(flag_n,flag_n+k):
        #         union_picture=union_proposal[flag_n:flag_n+k,:]
        #         IOU_score=box_iou(union_proposal[m].unsqueeze(0),union_picture,k)#IOU维度为 [一张图片的物体对个数，1]
        #         max_iou=torch.max(IOU_score,0)
        ...
        #pair_pred, obj_dists,pair_pos_embed,visual_rep,obj_feats,pair_obj_embed,num_rels, num_objs,logger,prod_rep= self.feature_generate(roi_features, proposals, num_objs, rel_pair_idxs, union_features, num_rels, logger)
        rel_feats, pair_pred, obj_dists= self.feature_generate(roi_features, proposals, num_objs, rel_pair_idxs, union_features, num_rels, logger)
        # #将新的pair_pos_embed传入！！！！
        # rel_feats=self.new_feature_generate(visual_rep, obj_feats, pair_obj_embed, pair_pos_embed, num_rels, num_objs,
        #                                      logger,prod_rep)


        rel_dists = self.rel_compress(rel_feats)
        # boxes=[]
        #pair_pos_embed行数为pair的个数，与rel_pair_idxs相对应
